...
from bot import collocations, formatter, main
...
def test_collocations_are_selected_in_order_without_repeating():
    ...
    first_key, first_card = collocations.next_card(store, state.posted)
    assert first_card.ausdruck == "eine Entscheidung treffen"
    state.record(first_key, "2026-09-24")
    assert collocations.next_card(store, state.posted)[1].ausdruck == "eine Pause machen"
